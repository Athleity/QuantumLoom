...
def test_creation():
    ...
    assert read_stab.stabilizer == stab
    assert read_stab.__class__.__name__ == "MeasureStabilizerSyndrome"
    # Test the loads/dumps both using the right class and the abstract base class
    assert read_stab == loads(MeasureStabilizerSyndrome, dumps(read_stab))
    assert read_stab == loads(Operation, dumps(read_stab))
    # Test the creation of MeasureObservable
    obs = PauliOperator(pauli="X", data_qubits=((0, 0),))
    read_obs = MeasureObservable(
        observable=obs,
    )
    assert read_obs.observable == obs
    assert read_obs.__class__.__name__ == "MeasureObservable"
    # Test the loads/dumps both using the right class and the abstract base class
    assert read_obs == loads(MeasureObservable, dumps(read_obs))
    assert read_obs == loads(Operation, dumps(read_obs))
