...
class TestEkaCircuitToCudaqConverter:
    """Test the EkaToCudaqConverter."""

    convert_expected_json = "convert_cudaq.json"
    emit_expected_json = "emit_cudaq.json"
    ...
    @property
    def converter(self) -> EkaToCudaqConverter:
    ...
    def test_emit_functions(
        ...
        with subtests.test(msg="Checking emit initialisation", case_id="init"):
            ...
            str_mismatch_msg = (
                "Initialization string does not match:\n"
                f"    - got      : {str_init}\n"
        ...
        for new_instruction_name, new_instruction in instructions_to_append.items():
            if not isinstance(new_instruction, Circuit):
                pytest.fail(
                    "Instruction to append must be a Circuit,"
