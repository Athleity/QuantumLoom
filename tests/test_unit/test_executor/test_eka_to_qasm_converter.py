...
from loom.executor.eka_to_qasm_converter import EkaToQasmConverter


class TestEkaToQasmConverter:
    """Unit tests for the EkaToQasmConverter class."""

    convert_expected_json = "convert_qasm.json"
    emit_expected_json = "emit_qasm.json"
    ...
    @property
    def converter(self) -> EkaToQasmConverter:
        """Return an instance of the EkaToQasmConverter."""
        return EkaToQasmConverter()

    def assert_converter_output_header(self, qasm_str_out: str):
        """Assert that the QASM output string has the correct header."""
        lines = qasm_str_out.splitlines()  # split string into lines
    ...
    def test_parsing_function(self):
        """Test the parse_target_run_outcome method of the converter."""

        reg_map = {
            Channel(label="c0", type=ChannelType.CLASSICAL): 0,
            Channel(label="c1", type=ChannelType.CLASSICAL): 1,
        }
        simulation_outcome = {"00": 1, "11": 1, "01": 1, "10": 1}
        expected_parsed = {"c0": [0, 1, 1, 0], "c1": [0, 1, 0, 1]}

        parsed = self.converter.parse_target_run_outcome((simulation_outcome, reg_map))
    ...
    def test_emit_functions(
        ...
        for new_instruction_name, new_instruction in instructions_to_append.items():
            if not isinstance(new_instruction, Circuit):
                pytest.fail(
                    "Instruction to append must be"
                    f" a Circuit, got {type(new_instruction)}"
                )
            with subtests.test(
