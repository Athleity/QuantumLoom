...
class TestRotatedSurfaceCodeApplicator(unittest.TestCase):
    ...
    # ,too-many-statements
    # def test_applicator_transversalhadamard(self):
    #     """
    #     Test that the applicator creates the correct circuit for a Transversal Hadamard
    #     operation.
    #     """
    #     lattice = Lattice.square_2d((10, 20))
    #     rsc_block = RotatedSurfaceCode.create(
    #         dx=3, dz=3, lattice=lattice, unique_label="q1"
    #     )
    #     test_block = RotatedSurfaceCode.create(
    #         dx=3,
    #         dz=3,
    #         lattice=lattice,
    #         unique_label="q1",
    #         x_boundary="VERTICAL",
    #         weight_4_x_schedule="Z",
    #     )
    #     thadamard_op = TransversalHadamard(rsc_block.unique_label)
    #     input_eka = Eka(lattice, blocks=[rsc_block], operations=[thadamard_op])
    #     base_step = InterpretationStep(
    #         block_history=((rsc_block,),),
    #         logical_x_operator_updates={
    #             rsc_block.logical_x_operators[0].uuid: (("c_(0, 0, 0)", 0),)
    #         },
    #         logical_z_operator_updates={
    #             rsc_block.logical_z_operators[0].uuid: (("c_(1, 0, 0)", 0),)
    #         },
    #     )
    #     output_step = RotatedSurfaceCodeApplicator(input_eka).apply(
    #         base_step, thadamard_op, same_timeslice=False, debug_mode=True
    #     )

    #     expected_circuit = Circuit(
    #         "transversal hadamard on q1",
    #         circuit=[
    #             [
    #                 Circuit(
    #                     "H",
    #                     channels=[Channel(label=f"{qubit}", type="quantum")],
    #                 )
    #                 for qubit in rsc_block.data_qubits
    #             ]
    #         ],
    #     )
    ...
    #     # Block history is updated
    #     self.assertEqual(len(output_step.block_history), 2)

    #     # There should only be 1 Block in the latest Block configuration
    #     self.assertEqual(len(output_step.block_history[-1]), 1)
    #     # The Block should have the same UUID as the original Block
    #     new_rsc_block = output_step.block_history[-1][0]
    #     self.assertEqual(
    #         output_step.block_history[0][0].unique_label, rsc_block.unique_label
    #     )
    #     self.assertEqual(new_rsc_block.unique_label, rsc_block.unique_label)
    #     self.assertEqual(new_rsc_block.unique_label, test_block.unique_label)

    #     # Check Stabilizer Transformation
    #     # Change in the Stabilizers should be recorded within the Stabilizer Evolution
    #     # of the InterpretationStep
    #     new_uuids = list(output_step.stabilizer_evolution.keys())
    #     # For this example all Stabilizers were changed

    #     # There should be the same number of new stabilizers as old stabilizers
    #     self.assertEqual(len(new_rsc_block.stabilizers), len(rsc_block.stabilizers))
    #     self.assertEqual(len(new_rsc_block.stabilizers), len(test_block.stabilizers))

    #     # Check the Schedules of the Stabilizers
    #     self.assertEqual(
    #         new_rsc_block.weight_4_x_schedule, test_block.weight_4_x_schedule
    #     )  # Final Block and Test Block should have the same X schedule
    #     self.assertEqual(
    #         rsc_block.weight_4_z_schedule, test_block.weight_4_x_schedule
    #     )  # The original and Test Block should have flipped schedules
    #     self.assertEqual(
    #         new_rsc_block.weight_4_z_schedule, test_block.weight_4_z_schedule
    #     )  # Same for Z schedule
    #     self.assertEqual(rsc_block.weight_4_x_schedule, test_block.weight_4_z_schedule)

    #     # The new stabilizers were recorded properly. (Can be found in Block and
    #     # stabilizer_evolution record)
    #     self.assertEqual(
    #         {each_stab.uuid for each_stab in new_rsc_block.stabilizers},
    #         set(new_uuids),
    #     )

    #     # Check that the Stabilizers have been transformed
    #     for new_stab in new_rsc_block.stabilizers:
    #         old_stab_uuid = output_step.stabilizer_evolution[new_stab.uuid][0]
    #         old_stab = next(
    #             each_stab
    #             for each_stab in rsc_block.stabilizers
    #             if each_stab.uuid == old_stab_uuid
    #         )
    #         # Check that the old and new Stabilizers share the same data and ancilla
    #         # qubits
    #         self.assertEqual(set(old_stab.data_qubits), set(new_stab.data_qubits))
    #         self.assertEqual(set(old_stab.ancilla_qubits), set(new_stab.ancilla_qubits))

    #         # Find identical Stabilizer in Test Block
    #         test_stab = next(
    #             each_stab
    #             for each_stab in test_block.stabilizers
    #             if each_stab.data_qubits == old_stab.data_qubits
    #         )
    #         self.assertEqual(set(test_stab.data_qubits), set(new_stab.data_qubits))
    #         self.assertEqual(
    #             set(test_stab.ancilla_qubits), set(new_stab.ancilla_qubits)
    #         )

    #         # Check that the Syndrome Circuits have been transformed
    #         self.assertEqual(
    #             len(new_rsc_block.syndrome_circuits), len(rsc_block.syndrome_circuits)
    #         )
    #         self.assertEqual(
    #             len(new_rsc_block.syndrome_circuits), len(test_block.syndrome_circuits)
    #         )

    #         # Check that all new Syndrome Circuits have different UUIDs.
    #         all_synd_uuids = [
    #             each_synd_circ.uuid
    #             for each_synd_circ in (
    #                 rsc_block.syndrome_circuits + new_rsc_block.syndrome_circuits
    #             )
    #         ]
    #         self.assertEqual(len(all_synd_uuids), len(set(all_synd_uuids)))

    #         new_synd_circ_uuid = new_rsc_block.stabilizer_to_circuit[new_stab.uuid]
    #         new_synd_circ = [
    #             each_circ
    #             for each_circ in new_rsc_block.syndrome_circuits
    #             if each_circ.uuid == new_synd_circ_uuid
    #         ]
    #         self.assertEqual(len(new_synd_circ), 1)
    #         new_synd_circ = new_synd_circ[0]

    #         # Check that the pauli has been successfully transformed
    #         if old_stab.pauli == "XX":
    #             self.assertEqual(test_stab.pauli, "ZZ")
    #             self.assertEqual(new_stab.pauli, "ZZ")
    #             self.assertEqual(new_synd_circ.pauli[-2:], "ZZ")
    #         elif old_stab.pauli == "ZZ":
    #             self.assertEqual(test_stab.pauli, "XX")
    #             self.assertEqual(new_stab.pauli, "XX")
    #             self.assertEqual(new_synd_circ.pauli[-2:], "XX")
    #         elif old_stab.pauli == "ZZZZ":
    #             self.assertEqual(test_stab.pauli, "XXXX")
    #             self.assertEqual(new_stab.pauli, "XXXX")
    #             self.assertEqual(new_synd_circ.pauli[-4:], "XXXX")
    #         elif old_stab.pauli == "XXXX":
    #             self.assertEqual(test_stab.pauli, "ZZZZ")
    #             self.assertEqual(new_stab.pauli, "ZZZZ")
    #             self.assertEqual(new_synd_circ.pauli[-4:], "ZZZZ")

    #     # Check that the Logical Operators have been transformed
    #     num_of_log_qubit = 1
    #     for each_log_qubit in range(num_of_log_qubit):
    #         # The Operators "flip" between X and Z
    #         old_x_logical_op = rsc_block.logical_x_operators[each_log_qubit]
    #         old_z_logical_op = rsc_block.logical_z_operators[each_log_qubit]
    #         new_x_logical_op = new_rsc_block.logical_x_operators[each_log_qubit]
    #         new_z_logical_op = new_rsc_block.logical_z_operators[each_log_qubit]
    #         test_x_logical_op = test_block.logical_x_operators[each_log_qubit]
    #         test_z_logical_op = test_block.logical_z_operators[each_log_qubit]
    #         # Check that the old and new Logical Operators share the same data qubits
    #         self.assertEqual(
    #             set(old_z_logical_op.data_qubits), set(new_x_logical_op.data_qubits)
    #         )
    #         self.assertEqual(
    #             set(old_x_logical_op.data_qubits), set(new_z_logical_op.data_qubits)
    #         )
    #         # The difference between the test block and the updated block (the block
    #         # afterwhich the transversal hadamard has been applied) is that the logical
    #         # operators in the test block are not flipped.
    #         # i.e.,
    #         # - XXX on (0, 0, 0), (1, 0, 0), (2, 0, 0) in test block in the
    #         # logical_x_operators attribute
    #         # - XXX on (0, 0, 0), (1, 0, 0), (2, 0, 0) in updated block in the
    #         # logical_z_operators attribute
    #         self.assertEqual(
    #             set(test_x_logical_op.data_qubits), set(new_x_logical_op.data_qubits)
    #         )
    #         self.assertEqual(
    #             set(test_z_logical_op.data_qubits), set(new_z_logical_op.data_qubits)
    #         )
    #         # Check that the pauli has been successfully transformed
    #         for each_old_op, each_new_op, each_test_op in zip(
    #             [old_z_logical_op, old_x_logical_op],
    #             [new_z_logical_op, new_x_logical_op],
    #             [test_z_logical_op, test_x_logical_op],
    #             strict=True,
    #         ):
    #             if each_old_op.pauli == "ZZZ":
    #                 self.assertEqual(each_test_op.pauli, "ZZZ")
    #                 self.assertEqual(each_new_op.pauli, "ZZZ")
    #             elif each_old_op.pauli == "XXX":
    #                 self.assertEqual(each_test_op.pauli, "XXX")
    #                 self.assertEqual(each_new_op.pauli, "XXX")

    #         # Check that logical_N_evolution has been updated within
    #         # InterpretationStep for both (N=x and N=z)
    #         self.assertEqual(
    #             output_step.logical_z_evolution,
    #             {new_x_logical_op.uuid: (old_z_logical_op.uuid,)},
    #         )
    #         self.assertEqual(
    #             output_step.logical_x_evolution,
    #             {new_z_logical_op.uuid: (old_x_logical_op.uuid,)},
    #         )

    #         # Check that logical updates of the new operators contains updates from the
    #         # previous logical operators for both

    #         self.assertEqual(
    #             output_step.logical_x_operator_updates.get(new_x_logical_op.uuid, ()),
    #             base_step.logical_z_operator_updates.get(old_z_logical_op.uuid, ()),
    #         )
    #         self.assertEqual(
    #             output_step.logical_z_operator_updates.get(new_z_logical_op.uuid, ()),
    #             base_step.logical_x_operator_updates.get(old_x_logical_op.uuid, ()),
    #         )


# TODO: Add in tests for all other operations (MoveBlock, AuxCNOT, LogicalPhaseViaYwall etc).  # pylint: disable=fixme
